...
Sync.prototype.getCalendars = function( calHome, callback ){
	...
	this.request( 'PROPFIND', { path : calHome, depth : 1 }, body.join(''), function( error, res ){
					...
					function( error, list ){
						...
						if( list ){
							...
							list = list.filter( function( item ){
								return item.resourcetype.indexOf('calendar') > -1;
							});
...
Sync.prototype.getCalendarHome = function( home, callback ){
	...
	this.request( 'PROPFIND', home, body.join(''), function( err, res ){
		...
		tools.parseXML( res.body, function( err, data ){
			...
			tools.find(
				...
				data,
				[ 'multistatus', 'response', 0, 'propstat', 0, 'prop', 0, 'C:calendar-home-set', 0, 'href', 0 ],
